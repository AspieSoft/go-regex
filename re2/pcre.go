...
func (reg *RegexpPCRE) RepFunc(str []byte, rep func(data func(int) []byte) []byte, blank ...bool) []byte {
...
func (reg *RegexpPCRE) RepStrComp(str []byte, rep []byte) []byte {
	ind := reg.RE.FindAllIndex(str, 0)
	...
	for _, pos := range ind {
		...
		r := []byte{}
		if reg, ok := regComplexSel.(*RegexpPCRE); ok {
		...
		}else if reg, ok := regComplexSel.(*Regexp); ok {
			r = reg.RepFunc(rep, func(data func(int) []byte) []byte {
				if len(data(1)) != 0 {
					return data(0)
				}
